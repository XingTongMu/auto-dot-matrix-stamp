...
这是一款基于小程序图像处理和 ESP32 硬件控制的自动点阵印章设备。用户上传图片后，小程序将其转换为黑白像素风图像，并通过蓝牙发送给印章设备；设备通过 XY 步进滑轨移动打印头，并控制 24V 电磁铁带动微型海绵印头逐点下压，从而在纸面上自动印出像素化图案。
...
> 小程序一次性发送完整点阵数据，ESP32 本地执行完整打印任务。

不推荐通过蓝牙实时逐步控制电机，因为蓝牙通信可能存在延迟、丢包或卡顿，容易导致运动中断和打印失败。
...
由于像素间距约为 2 mm，印头实际尺寸不宜直接做成 2 mm，建议控制在：

```text
1.2 mm ～ 1.6 mm
```

这样可以预留墨水扩散空间，避免相邻像素点粘连成片。

---

### 8.4 电磁铁驱动电路

ESP32 不能直接驱动电磁铁，需要通过 MOSFET 控制。
...
第一版 MVP 不追求复杂功能，目标是先验证完整技术闭环：

> 图片点阵化 → 蓝牙传输 → XY 定位 → 电磁铁打点 → 成像效果
...
| 难点 | 说明 |
|---|---|
| 点位精度 | 2 mm 像素间距要求滑轨定位稳定 |
| 墨水扩散 | 印头过大或墨水过多会导致糊点 |
| 电磁铁发热 | 高频打点时电磁铁不能长时间通电 |
| 印头复位 | 断电后必须快速抬起，否则会拖墨 |
| 纸张固定 | 纸面不平会影响每个点的深浅 |
| 蓝牙稳定性 | 不应依赖蓝牙实时控制每一步动作 |
| 机械振动 | 电磁铁下压会对滑台产生冲击 |
| 电源干扰 | 电机和电磁铁开关可能影响 ESP32 稳定性 |
...
本项目涉及 24V 电源、步进电机驱动器和电磁铁等感性负载，调试时需要注意安全。

注意事项：

- 不要用 ESP32 GPIO 直接驱动电磁铁
- 电磁铁必须使用 MOSFET 驱动
- 电磁铁两端需要添加续流二极管
- ESP32、电机驱动器、电磁铁驱动电路必须共地
- 检查 24V 电源电流是否足够
...
> 搭建一台可以在 80 mm × 80 mm 范围内打印 40 × 40 黑白点阵图案的自动点阵印章原型机。

第一版重点验证技术可行性，不追求商业化量产。
